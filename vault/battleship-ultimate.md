...
| A29 | Single-training-path bottleneck | Training | 🟡 | ✅ | Resolved: added `cycle_all_rooms_parallel` binary using pthreads; original `cycle_all_rooms` kept sequential. Phase 3 now has a parallel path. |
...
| A41 | No cross-validation strategy | Training | ⚪ | ✅ | IMPLEMENTED: `multi_market_trainer --validate` runs 5-fold walk-forward validation with summarizing IS vs OOS WR and overfit detection. |
...
| A59 | No multi-threaded room cycling | Training | ⚪ | ✅ | Implemented by A29: `cycle_all_rooms_parallel` binary built and verified. |
...
| B34 | No autoencoder for unsupervised features | Features | ⚪ | ✅ | **FIXED**: feature_autoencoder.c — rank-1 power-iteration latent projection. Reads docs/data/latest_features.json, writes latents + reconstructed feature JSON. Binary-only; no engine struct or N_FEATURES changes. |
...
| C39 | No portfolio-level VaR model | Risk | 🟡 | ⏳ | PARTIAL. risk_report.c + risk_analytics.c compute trade-level/agent-level VaR (95/99) and Expected Shortfall from trade_log.csv and RoomState history. GAP: no explicit portfolio VaR rollup aggregating risk across multiple rooms (`docs/data/risk_metrics.json` exists, but not a cross-room portfolio VaR). |
...
| F13 | No monitoring dashboard beyond CLI | Infra | 🟡 | ✅ | **STALE**: monitoring stack exists — CLI `./room_health_monitor`, web API `api_server` (port 9091), static data server `data_server` (9090), JSON logs `engine_log.jsonl`, `cycle_metrics.jsonl`, plus `dashboards/` website pages. |
