...
- **Battleship:** 365 cells across 9 domains (1 🔴 P0, 83 🟡 P1, 183 ⚪ P3)
- **Rooms:** 16 configured. market_type set per room. SIGMA_NORMALIZER fixed (0.001→0.15). 98% agent voting. 34 features (OB, CVD, 7 derived, time, options, BTC-SP500 corr, VIX regime). Warm-start from elite genomes on restart. Daily loss limit (C05, 10% max). Cycle count persists (A57). Heartbeat/alert file (A58). Feature staleness detection (B37). Epsilon-greedy exploration (A30, ε=0.05→0.005). Multi-objective Darwin fitness (A40, WR+PNL+DD+freq). Elite preservation (A52, top 5%). Per-cycle metrics JSONL (A56). Cycle/epsilon in RoomState (types.h).
...
- **Latest batch:** A30 epsilon exploration, A40 multi-objective Darwin, A52 elite preservation, A56 cycle metrics logging, A60 watchdog reporting. P1: 87→83, P3: 196→183.
